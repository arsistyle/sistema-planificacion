...
function automatizar_init() {
  ...
  $(':text, textarea').each(function (i, el) {
    ...
    var validar = 1;
    if ($(el).hasClass('requerido')) {
      for (let i in tiposNoConsiderados) {
  ...
  if (tipo == 'tipo_nombre') {
    ...
    $(el).blur(function () {
      ...
      if (
        ...
        ($(this).data('placeholder') && this.value != $(this).data('placeholder'))
      ) {
...
function formatearRut(campo) {
  let str = new String(campo.value);
  ...
  if (dv.length && str.length) {
    let n = [];
    str = str.split('');
    let count = Math.ceil(str.length / 3) - 1;
    for (let i = str.length - 1; i >= 0; i -= 3) {
      let num1 = str[i] ? str[i] : '';
      let num2 = str[i - 1] ? str[i - 1] : '';
      let num3 = str[i - 2] ? str[i - 2] : '';
...
/* FUNCIONES ON DEMAND ************/
export function validarFormulario(id_formulario, variables) {
  ...
  for (let i in variables.otrosCampos) {
...
function limpiarFormulario(id_formulario) {
  ...
  $(
    '#' + id_formulario + ' :text, #' + id_formulario + ' textarea, #' + id_formulario + ' select'
  ).each(function (i, el) {
...
export default automatizar_init;
